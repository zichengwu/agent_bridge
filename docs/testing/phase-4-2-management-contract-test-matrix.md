...
| SEC-003 | Security    | 写操作/会话交换的 Origin 缺失或不同源；SSE/preview 的 Origin 不同源；原生同源安全 GET 缺失 Origin  | 前两类返回 ORIGIN_REJECTED；后一类仅在精确 Host、`Sec-Fetch-Site: same-origin` 与路由专属会话/媒体类型/客户端标记/stream 门禁全部满足时通过；始终不返回 CORS 允许头 |
...
| SEC-007 | Security    | 写请求缺 Cookie、CSRF 或精确 Origin；普通 JSON 读与 preview 缺同源标记；SSE/preview 缺路由专属门禁 | 分别稳定拒绝，业务状态不变化                                                                                                                                        |
