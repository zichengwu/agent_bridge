...
export interface IsolatedOpenCodeEnvironment {
...
export async function createIsolatedOpenCodeEnvironment(input: {
  readonly root: string;
  readonly workDirectory: string;
  readonly originalEnvironment: NodeJS.ProcessEnv;
}): Promise<IsolatedOpenCodeEnvironment> {
  if (process.platform !== "darwin") {
    throw new Error("OPENCODE_B_SIMULATED_REQUIRES_MACOS_SANDBOX");
  }
  ...
  const codexHome = join(isolationRoot, "codex-home");
  ...
  await writeFile(openCodeConfigPath, `${JSON.stringify(SAFE_CONFIG, null, 2)}\n`, "utf8");
  ...
  const realExecutable = await realpath(join(packageRoot, "node_modules/.bin/opencode"));
  ...
  await writeFile(executablePath, wrapper, { encoding: "utf8", mode: 0o700 });
  ...
  const environment: Record<string, string> = {
    ...
    LC_ALL: input.originalEnvironment.LC_ALL ?? "C.UTF-8",
    CI: "1",
    NO_COLOR: "1",
    ...
    OPENCODE_CONFIG_CONTENT: JSON.stringify(SAFE_CONFIG),
  };
  ...
  return {
    ...
    cleanup: () => rm(isolationRoot, { recursive: true, force: true }),
  };
}
...
export async function assertRootRemoved(root: string): Promise<void> {
  await access(root).then(
    () => {
      throw new Error("OPENCODE_B_SIMULATED_TEMP_ROOT_REMAINS");
    },
    () => undefined,
  );
}
